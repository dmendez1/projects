...
public class MouseRace {
	...
	public MouseRace() {
		...
		System.out.println("PROGRAMMING ASSIGNMENT 5\n");
		System.out.println("**************************************");
	...
	private boolean gameLost() {
		if (mouse.x == cat.x && mouse.y == cat.x) {
			return false;
		} else
			return true;
	}
	...
	private void makeMove(int x, int y, Animal animal) {
		...
		} else if (board[x][y] == "#" && animal.representation == "C"){
	...
	private void parseCmd(String cmd, Animal animal) {
		...
		case "U":
			...
			case "N":
				parseCmd("S", animal);
				parseCmd("S", MouseRace.cat);
				score++;
				break;
			case "S":
				parseCmd("N", animal);
				parseCmd("N", MouseRace.cat);
				score++;
				break;
			case "E":
				parseCmd("W", animal);
				parseCmd("W", MouseRace.cat);
				score++;
				break;
			case "W":
				parseCmd("E", animal);
				parseCmd("E", MouseRace.cat);
				score++;
				break;
			default:
				break;
	...
	// Main method that creates the MouseRace object
	public static void main(String[] args) {
		MouseRace game = new MouseRace();
	}
